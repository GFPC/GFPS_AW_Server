...
**`PUT /api/1/manager/invitations`** — Bronevik в теле (`token`, `u_hash`). Для каждой строки приглашения: ключ — **строка с числовым id** (например **`"42"`**), значение **`{"data": ...}`** — JSON-объект или **`null`** (очистить); старое значение **полностью заменяется**. Одна или несколько строк в одном запросе. Ответ: `data.results[]` (по каждому id — `invitation` или `error: not_found`).
