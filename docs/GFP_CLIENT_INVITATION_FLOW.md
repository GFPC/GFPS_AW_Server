...
- **Токен** возвращается и при **создании**, и в **списке** приглашений (один и тот же Base58), чтобы менеджеры в SaaS могли снова сообщить его сотруднику. У **очень старых** строк (до миграции с колонкой секрета в БД) в списке `token` может быть **`null`** — тогда нужно выдать новое приглашение.
...
- Опционально в каждой строке **`data`**: произвольный JSON-объект (флаги «письмо отправлено», черновик и т.д.); сервер хранит как opaque JSON.
...
**Клиент менеджера:** показывайте **`data.invitations[i].token`** в UI (его можно снова взять из **списка** приглашений). Для строк с `token: null` (наследие БД) — только выдать новое приглашение.
...
**Успех:** `data.invitations[]` — в каждой строке тот же **`token`** (Base58), что при создании, если секрет сохранён в БД; иначе `token: null` (редкий случай старых данных).

Используйте также для статусов (`installed`, `installed_at`, `email`) и поля **`data`**.

### Обновление `data` без пересоздания приглашения

**`PUT /api/1/manager/invitations/<id>`** — Bronevik в теле (`token`, `u_hash`). Обязателен ключ **`data`**: JSON-объект или **`null`** (очистить). Старое значение **полностью заменяется**. Ответ: `{"status":"success","data":{"invitation":{...}}}`. Неверный `id` → `404`.

**`PUT /api/1/manager/invitations`** (без id в пути) — то же для **нескольких** строк: в JSON, кроме `token`/`u_hash`, ключи вида **`"42"`**, **`"43"`** (id строк), значения **`{"data": { ... }}`**. Ответ: `data.results[]` с результатом по каждому id (в т.ч. `not_found` для отдельных id без отката остальных).
...
```json
{
  ...
  "invitation": {
    "id": 1,
    "token": "BASE58_OR_NULL_FOR_LEGACY_ROWS",
    "installed": true,
    "user_id": 42,
    "..."
  }
